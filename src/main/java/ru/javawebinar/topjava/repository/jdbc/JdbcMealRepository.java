...
//    @Autowired
    public JdbcMealRepository(JdbcTemplate jdbcTemplate, NamedParameterJdbcTemplate namedParameterJdbcTemplate) {
        this.mealInsert = new SimpleJdbcInsert(jdbcTemplate).withTableName("meals").usingGeneratedKeyColumns("id");
    ...
    public Meal save(Meal meal, int userId) {
        ...
        if (meal.isNew()) {
            Number newId = mealInsert.executeAndReturnKey(map);
            meal.setId(newId.intValue());
        } else if (namedParameterJdbcTemplate
                .update("UPDATE meals SET id=:id, " +
                        "description=:description, calories=:calories, date_time=:date_time " +
