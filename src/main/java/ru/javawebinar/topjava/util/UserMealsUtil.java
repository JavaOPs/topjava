...
//        System.out.println(filteredByStreams(meals, LocalTime.of(7, 0), LocalTime.of(12, 0), 2000));
    ...
    public static List<UserMealWithExcess> filteredByStreams(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        ...
        Map<LocalTime, Integer> sumCaloriesByDate = meals.stream()
                .collect(Collectors.groupingBy(um -> um.getDateTime().toLocalTime(), Collectors.summingInt(UserMeal::getCalories)));
        System.out.println(sumCaloriesByDate);
        return meals.stream().filter(um -> TimeUtil.isBetweenHalfOpen(um.getDateTime().toLocalTime(), startTime, endTime))
                .map(um -> new UserMealWithExcess(um.getDateTime(), um.getDescription(), um.getCalories(),
                        sumCaloriesByDate.get(um.getDateTime().toLocalTime()) > caloriesPerDay))
